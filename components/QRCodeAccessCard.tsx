...
export function QRCodeAccessCard({
  ...
  return (
    <article
      className={
        isEvaluation
          ? `rounded-[1.5rem] border border-[#5d7896]/55 bg-[#081725] text-white shadow-[0_24px_70px_rgba(2,10,22,0.34)] ${
              compact ? "p-4" : "p-5"
            }`
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <h3 className={isEvaluation ? "mt-3 text-xl font-bold text-white" : "mt-3 text-2xl font-bold text-navy-900"}>
            {title}
          </h3>
          <p className={isEvaluation ? "mt-1 text-base font-semibold text-[#d7ecff]" : "mt-1 text-lg font-semibold text-navy-700"}>
            {subtitle}
          </p>
        </div>
      </div>
      ...
      >
        ...
        <p className={isEvaluation ? "mt-4 text-center text-base font-semibold leading-7 text-white/[0.82]" : "mt-4 text-center text-sm font-semibold leading-6 text-slate-600"}>
      ...
      <p className={isEvaluation ? "mt-3 text-center text-base font-semibold leading-7 text-white/[0.78]" : "mt-3 text-center text-sm font-semibold leading-6 text-slate-600"}>
        {isEvaluation
          ? "One laptop per group. Work is saved only on that laptop/browser."
          : "One laptop per group is recommended. Work is saved only on that laptop/browser."}
      </p>
    </article>
  );
}
